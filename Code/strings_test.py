...
class StringsTest(unittest.TestCase):
    ...
    def test_contains_with_matching_patterns(self):
        ...
        # TODO: Write more positive test cases with assert is True statement
        assert contains('bacb', 'bac') is True
        assert contains('bbb', 'bb') is True
        assert contains('Bobby', 'bob') is True
        assert contains('mississippi', 'sis') is True
        # ...
    ...
    def test_contains_with_non_matching_patterns(self):
        ...
        assert contains('toy', 'boy') is False
        assert contains('baby', 'maybe') is False
        assert contains('ball', 'wall') is False
        assert contains('car', 'mar') is False
        # ...
    ...
    def test_find_index_with_matching_patterns(self):
        ...
        assert find_index('call', 'all') == 1
        assert find_index('already', 'ready') == 2
        assert find_index('papercut', 'paper') == 0
        assert find_index('dolphin', 'in') == 5
        # ...
    ...
    def test_find_index_with_non_matching_patterns(self):
        ...
        assert find_index('cat', 'dog') is None
        assert find_index('large', 'small') is None
        assert find_index('dog', 'god') is None
        assert find_index('love', 'evil') is None
        # ...
    ...
    def test_find_all_indexes_with_matching_patterns(self):
        ...
        assert find_all_indexes('xyz', 'yz') == [1]
        assert find_all_indexes('babababy', 'ba') == [0, 2, 4]
        assert find_all_indexes('Hubbabubba', 'ubba') == [1, 6]
        assert find_all_indexes('crazy', 'razy') == [1]
        # ...
    ...
    def test_find_all_indexes_with_non_matching_patterns(self):
        ...
        assert find_all_indexes('corn', 'a') == []
        assert find_all_indexes('boomer', 'tu') == []
        assert find_all_indexes('qwerty', 'gas') == []
        assert find_all_indexes('xyz', 'abc') == []
        # ...
